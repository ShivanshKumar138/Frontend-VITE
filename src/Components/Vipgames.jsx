...
const App = () => {
  ...
  const handleTabChange = (event, newValue) => {
    ...
    if (tabsRef.current) {
      ...
      if (tabElements[newValue]) {
        tabElements[newValue].scrollIntoView({ behavior: "smooth", inline: "center" });
      }
    }
...
return (
  ...
  <AppBar position="sticky" sx={{ background: "#4781FF", height: 40, justifyContent: "center" }}>
    <Toolbar sx={{ minHeight: "40px !important", display: "flex", justifyContent: "space-between", padding: "0 10px" }}>
      <h3 style={{ margin: 0, fontSize: "16px", color: "white" }}>Game</h3>
  ...
  <Box sx={{ flexGrow: 1 }}>
    <AppBar position="static" color="default">
      <Tabs
        ...
        ref={tabsRef} // Reference for scrolling
      >
        {[
          ...
          <Tab
            ...
            label={
              <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                <img src={jdb} alt={tab.label} width="30" height="25" />
                {tab.label}
              </Box>
            }
          />
